...
public class Order extends Observable {
	...
	public String orderTooText(Map<Menu, Integer> map) {
		String text = "";
		for (Map.Entry<Menu, Integer> order : map.entrySet()) {
			Menu menu = order.getKey();
			...
			String date =menu.getCreatedDate();





			text += String.format("%5d   %-28s %5d     %s\n", qty,name , price,date );

		}

		return text;
	}
	public String orderToooText(Map<Menu, Integer> map) {
		String text = "";
		for (Map.Entry<Menu, Integer> order : map.entrySet()) {
			Menu menu = order.getKey();
			...
			String name = menu.getName();
			int price = menu.getPrice() * qty;
			String tab=menu.getTable();

			String date =menu.getCreatedDate();

			text += String.format(" %s* %5d  %-28s %5d      %s\n",tab, qty,name , price,date );

		}

		return text;
	}
	public String ordeerToooText(Map<Menu, Integer> map) {
		String text = "";
		for (Map.Entry<Menu, Integer> order : map.entrySet()) {
			Menu menu = order.getKey();
	...
	public int getTotalexpense(Map<Menu, Integer> map) {
		tmpTotal = 0;
		map.forEach((k, v) -> tmpTotal += k.getRprice() * v);
		return tmpTotal;
	}
